...
abstract class NativeBatchScanOrcBase(batchScanExec: BatchScanExec)
    extends NativeBatchScanBase(batchScanExec) {
  ...
  override def doExecuteNative(): NativeRDD = {
    ...
    val projection = schema.map(field => getDataSchema.fieldIndex(field.name))
  ...
  override val nodeName: String =
    s"NativeOrcScan ${batchScanExec.table.name()}"
}
